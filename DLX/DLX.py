...
def solution(filename):
    ...
    for da in Data:
        ...
        data.append(round(end_time - start_time, 5))
        if("easy" in filename):
            csv.writ_data(data, "DLX/DLX_easy.csv")
        else:
            csv.writ_data(data, "DLX/DLX_hard.csv")
        print(solved)
...
def solute_Soduko(n):
        ...
        for i in range(len(ss.answer)):
            if(len(a) == 9):
                solved.append(a)
                a = []
            a.append(int(chr(ss.answer[i] + ord('1'))))
        return np.asarray(solved)


def run(difficulty):
    if difficulty == "easy":
        solution("Sudoku/easy.txt")
    if difficulty == "hard":
        solution("Sudoku/hard.txt")
